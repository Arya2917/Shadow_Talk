  ...
  return (
    <Layout>
      <Head>
        ...
        <meta name="description" content="Get help and support for Shadow Talk. Find answers to common questions." />
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              ...
              <h2 className="text-3xl font-bold text-white mb-6 mt-12">Popular Support Topics</h2>
            ...
            <div className="bg-slate-800 p-6 rounded-lg shadow-lg border border-slate-700 h-fit">
              ...
              <div className="mt-8 pt-6 border-t border-slate-700">
                ...
                <p className="text-slate-300">Saturday - Sunday: 10AM - 6PM EST</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="bg-slate-800 rounded-lg shadow-lg border border-slate-700 overflow-hidden">
      ...
      >
        ...
        <ChevronDown className={`w-5 h-5 text-purple-500 transform transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      {isOpen && (
    ...
    <div className="bg-slate-800 p-4 rounded-lg shadow-lg border border-slate-700">
      ...
      <ul className="space-y-2">
        {items.map((item, index) => (
          <li key={index} className="text-slate-300 hover:text-purple-400 transition-colors cursor-pointer">
            {item}
          </li>
        ))}
      </ul>
...
const ContactMethod = ({ icon, title, description, actionText, isPremium = false }) => {
  return (
    <div className="flex items-center gap-4 p-4 bg-slate-700 rounded-lg">
      ...
      <button className={`px-3 py-1 rounded text-sm font-medium ${isPremium ? 'bg-slate-600 text-slate-300' : 'bg-purple-600 text-white hover:bg-purple-700'} transition-colors`}>
        {actionText}
      </button>
    </div>
  );
